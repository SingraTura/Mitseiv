...
export class ManagerQuedadaService {
  ...
  public enviarMensajeAlChat(idQuedada: string, mensajero: string, mensaje: string) {
  ...
  public obtenerListaQuedadas(usuarioActual: string): Array<Quedada> {
    const resultado = Array<Quedada>();
    this.quedadas.forEach(quedada => {
      if (quedada.creador === usuarioActual) {
  ...
  public obtenerListaQuedadasPublicas(): Array<Quedada> {
    const resultado = Array<Quedada>();
    MiConsola.alerta(this.quedadas.length.toString())
    this.quedadas.forEach(quedada => {
